...
const myAge  = 7
let priceMenssage = null
const isChild = myAge <=7
const isOld = myAge >=65

if( isChild || isOld ){
    priceMenssage = 'Para você, a entrada é grátis!'
}else {
    priceMenssage = 'A entrada é R$ 30,00.'
}
console.log(priceMenssage)
/*
  04
...
for(let i = 0; i < numbers.length; i++) {
    ...
    const numberBetween = number >= 11 && number <= 90
    if(numberBetween){
...
for(let i = 0;i < crazyArray.length; i++) {
        ...
        const typeofBoolean = typeofItem === 'boolean'
        const typeofNumber = typeofItem  === 'number'
...
const numberImparString = arrayImpar.join(', ').replace(', 3', ' e 3')
const numberParString = arrayPar.join(', ').replace(', 5', ' e 5')
console.log(`Numeros ímpares: ${numberImparString}. Números pares: ${numberParString}`)