...
elementsInsideDiv.forEach(element => {
  element.addEventListener('click', event => {
    ...
    h2.textContent = `Clicou no ${event.target.tagName.toLowerCase()}, filho
    da div.`
  })
})
...
const eggContent = document.querySelector('.egg')
const showCoordenates =  ({offsetX , offsetY}) => {
    eggContent.textContent = `Eixo X ${offsetX} | Eixo Y ${offsetY}`
}

eggContent.addEventListener('mousemove', showCoordenates)
...
const changeColorEgg = () => {
    eggContent.style.backgroundColor = "lightgoldenrodyellow"
}

bt.addEventListener('click', changeColorEgg )
/*
  07
...
const devFront = people.some(({profession}) => profession === 'Front-end developer')
    if(devFront) {
