...
- Ao invés de inserir manualmente a posição do último caractere, faça com que a obtenção da última letra funcione para 
...
const minuscula = summerMessage.toLowerCase()
...
const maiuscula = winterMessage.toUpperCase()
...
const indexShow = winterMessage.lastIndexOf('n')
...
console.log(`O nº de episódios é: ${episodes}`)
...
- A template string deverá resultar no mesmo valor que a "newName" tinha anteriormente (nome e sobrenome com letras 
    iniciais maiúsculas);
- Não digite a nova string manualmente.
- Não modifique a string da const "name";
- Exiba a "newName" no console;
