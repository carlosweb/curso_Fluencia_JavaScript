...
const nameAnimals3Letters = animals.filter(({length}) => length === 3 )
console.log(nameAnimals3Letters)
...
*/
 ...
 const counterLettersAnimals = animals.map(pet => pet.length)
 console.log(counterLettersAnimals)
...
const peopleNearMe = ({nearMe}) => nearMe 
const friendsNearMe = friends.filter(peopleNearMe).map(({ name }) => name)
...
const numImpar = number => number % 2 === 1
const numbersImpares = numbers
  .filter(numImpar)
...
const sumAllCountriesLessChina = data
  .filter(({country}) => country !== 'China')
  .reduce((acc, {population}) => acc + population, 0)
console.log(sumAllCountriesLessChina)